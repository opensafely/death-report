...
table_by_subgroup <- dataset_measure %>% 
...
#Global Mortality rate by year and source global
ggplot(dataset_measure[dataset_measure$subgroup_cat== "overall",], aes(x = year, y = numerator, color = data_source)) +
  geom_line() +
  labs(x = "Year", y = "Number of deaths", color = "Source") +
  theme_minimal()
